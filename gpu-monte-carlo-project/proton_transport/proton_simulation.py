#!/usr/bin/env python3
"""
GPU Monte Carlo Dose Engine: Proton Transport Module
Proton transport simulation with energy deposition and scattering.
"""
...
class ProtonTransport:
    """Proton transport simulation for Monte Carlo dose calculation."""
    
    def __init__(self, config: Dict = None):
        """Initialize proton transport."""
    ...
    def calculate_proton_transport(self) -> Dict[str, any]:
        """Calculate proton transport simulation."""
        print("Calculating proton transport simulation...")
        
        # Initialize protons
        protons = self._initialize_protons()
        ...
        for i, proton in enumerate(protons):
            track = self._simulate_proton_track(proton, phantom)
            proton_tracks.append(track)
        ...
        efficiency = self._calculate_transport_efficiency(protons, proton_tracks)
        
        self.transport_results = {
            'dose_map': dose_map,
            'energy_deposition': energy_deposition,
            'proton_tracks': proton_tracks,
    ...
    def _initialize_protons(self) -> List[Dict]:
        """Initialize proton data."""
        protons = []
        num_protons = self.config['num_protons']
    ...
    def _simulate_proton_track(self, proton: Dict, phantom: Dict) -> Dict:
        ...
        track = {
            ...
            'total_distance': 0.0,
            'final_energy': proton['energy']
        }
        
        current_position = proton['position'].copy()
        current_direction = proton['direction'].copy()
        current_energy = proton['energy']
        ...
        for step in range(max_steps):
            ...
            # Check if proton is absorbed or escaped
    ...
    def _calculate_transport_efficiency(self, protons: List[Dict], 
                                      tracks: List[Dict]) -> float:
        """Calculate transport efficiency."""
        total_protons = len(protons)
        successful_tracks = sum(1 for track in tracks if len(track['steps']) > 0)
        
        return successful_tracks / total_protons if total_protons > 0 else 0.0
    ...
    def calculate_performance_metrics(self) -> Dict[str, float]:
        ...
        efficiency = self._calculate_transport_efficiency(
            self.transport_results.get('proton_tracks', []),
            self.transport_results.get('proton_tracks', [])
        )
    ...
    def plot_transport_analysis(self, output_dir: str = "proton_transport") -> None:
        ...
        axes[0, 0].set_title('Proton Transport - Dose Map')
        ...
        # Particle tracks
        tracks = self.transport_results['proton_tracks']
        if tracks:
            ...
            axes[1, 1].set_title('Proton Tracks')
...
def main():
    """Main function to demonstrate proton transport."""
    print("GPU Monte Carlo Dose Engine: Proton Transport")
    print("=" * 60)
    ...
    print("Proton transport complete!")
